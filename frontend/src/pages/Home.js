    ...
    <div className="home-page">
      <section className="home-section home-section-one">
        ...
        <div className="home-section-inner">
          {" "}
          {/* Новый внутренний контейнер */}
          <div className="content">
            <h1 className="title">
              <div className="letters">
                {" "}
                {/* Контейнер для стилей букв */}
                {titleStructure.map((line, lineIndex) => (
                  <div key={lineIndex}>
                    {" "}
                    {/* Контейнер для каждой строки */}
                    {line.map((word, wordIndex) => (
                      <React.Fragment key={`${lineIndex}-${wordIndex}`}>
                        {" "}
                        {/* Fragment для группировки слов в строке */}
                        {word.split("").map((letter, letterIndex) => (
                          <span
                            ...
                            style={{
                              "--index":
                                letterIndex /* z-index на основе индекса буквы в слове */,
                              zIndex:
                                letterIndex /* z-index на основе индекса буквы в слове */,
                              transitionDuration: `${
                                letterIndex / 5 + 1
                              }s` /* transition-duration на основе индекса буквы в слове */,
                            }}
                          >
                            {letter}
                          </span>
      ...
      <section ref={secondSectionRef} className="home-section home-section-two">
        {" "}
        {/* Вторая секция */}
        <div className="home-section-inner">
          {" "}
